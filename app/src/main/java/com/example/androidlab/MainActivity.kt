...
class MainActivity : AppCompatActivity(), RecyclerViewClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val initDBThread = Thread {
            ...
            Log.i("AddMoviesToList", "Movies in MovieList: $movieList")
        ...
        runBlocking {
            waitForThread(initDBThread)
            initRecycleView()
        }
    }

    private suspend fun waitForThread(dbThread : Thread) = coroutineScope{
            launch {
                ...
                while (true) {
                    if (dbThread.isAlive) {
                        ...
                        milliseconds++
                        total += milliseconds
                        Log.i("WaitingForThread", "Have waited $total" + "ms, waiting: $milliseconds" + "ms next")
                    } else {
                        break
                    }
                }
    ...
    private suspend fun initDataBase() {
        ...
        while (deleteAllThread.isAlive) {
            waitForThread(deleteAllThread)
        }
